...
CSV_Path1 ="../data_augmentation/nlpaug_data_grav.csv"
CSV_Path2 ="../data_augmentation/nlpaug_data_ses.csv"
LANG = 'en'
OLANG = 'es'
API = 'google'
...
def maind(grav = True):
    if grav:
        df = pd.read_csv(CSV_Path1, header = 0)
        ...
        df = pd.read_csv(CSV_Path2, header = 0)

        x = df['Item (Texto)'].values
        y = df['GravedadMode'].values
        
        X_train, X_test, y_train, y_test = train_test_split(x,y,test_size = 0.25, random_state = 0)
        df2 = pd.DataFrame(columns = ['Item (Texto)' , 'SesgoMode'])
        df3 = pd.DataFrame(columns = ['Item (Texto)' , 'SesgoMode'])
        for i in range(len(X_train)):
            df2.loc[i] = [X_train[i],y_train[i]]
        for i in range(len(X_test)):
            df3.loc[i] = [X_test[i],y_test[i]]
            ...
            df_concat = pd.concat([df2, translate(df2, LANG, OLANG)], ignore_index=True, sort=False)
        df_concat.to_csv(f'../data_augmentation/Train-{API}-{LANG}_ses.csv')
        df3.to_csv(f'../data_augmentation/Test-{API}-{LANG}_ses.csv')
